...
class Optimizer:
    def __init__(
        ...
        bounds = []
        for i in range(len(self._linear_coefficients.linear_coefficients)):
    ...
    def run(self) -> np.ndarray:
        res = linprog(
            self._linear_coefficients.linear_coefficients,
            A_ub=self._ineq_matrix_constr.inequality_matrix_constraint,
            b_ub=self._ineq_vec_constr.inequality_vector_constraint,
            bounds=self._bounds,
            method="highs",
            options=None,
            integrality=np.ones(len(self._linear_coefficients.linear_coefficients)),
        )
        if not res.success:
